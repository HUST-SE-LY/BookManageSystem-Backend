...
app.use(
  koaJwt({
    ...
    path: [/^\/api\/user\/login/, /^\/api\/user\/register/, /^\/api\/supplier/, /^\/api\/admin/,/^\/api\/user_type/],
  })
);
...
app.use(koaJwt({
  ...
  path:[/^\/api\/user/, /^\/api\/supplier/,/^\/api\/admin\/login/ ,/^\/api\/user_type/]
}))
...
app.use(koaJwt({
  ...
  path:[/^\/api\/user/, /^\/api\/admin/,/^\/api\/supplier\/login/, /^\/api\/supplier\/register/,/^\/api\/user_type/]
}))
  ...
  if (ctx.url.endsWith("register") || ctx.url.endsWith("login")) {
    ...
    if (parts.length === 2) {
      ...
      if (/^Bearer$/i.test(scheme)) {
        ...
        try {
          let secret;
          (ctx.url.startsWith('/api/user') || type === 'user') && (secret = process.env.SECRET as string);
          (ctx.url.startsWith('/api/admin') || type === 'admin') && (secret = process.env.ADMIN_SECRET as string);
          (ctx.url.startsWith('/api/supplier') || type === 'supplier' )&& (secret = process.env.SUPPLIER_SECRET as string);
...
SupplyAuthor.sync();
SupplyKeyword.sync();
Supply.belongsToMany(Author, {through: SupplyAuthor});
Supply.belongsToMany(Keyword, {through: SupplyKeyword});
Author.belongsToMany(Supply, {through: SupplyAuthor});
Keyword.belongsToMany(Supply, {through: SupplyKeyword});
